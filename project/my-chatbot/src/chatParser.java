...
{
	public void summary() //This method prints the summary of all chats
	{
		try
		{
			...
			fileProcessorBE.LogChatAndPrint(String.format("There are %d total chats to date with user asking %d times and system respond %d times. Total duration is %d seconds.\n\t", 
	...
	public void chatSummary(int chatNum)
	{
		try
		{
			FileReader reader = new FileReader("data/chat_sessions/chat_statistics.csv");
			...
			if(numSystemMessage == 0) //There is always at least 1 system message, so if it's 0 then its an invalid chat num
				fileProcessorBE.LogChatAndPrint(String.format("Error, there is no chat with the ID num of %d\n\t",chatNum));
			else
				fileProcessorBE.LogChatAndPrint(String.format("Chat %d has user asking %d times and system respond %d times. Total duration is %d seconds.\n\t", 
	...
	public void showChat(int chatNum) //This method gets the file of the chat needed to print, and sends that file to the print contents method
	{
		try
		{
			FileReader reader = new FileReader("data/chat_sessions/chat_statistics.csv");
	...
	public void printFileContents(String fileName) //This method prints all the contents of a given file
	{
		try
		{
			...
			{
				fileProcessorBE.LogChatAndPrint(line+"\n"); //print each line of the file
			}
			fileProcessorBE.LogChatAndPrint("\t");
